...
**Prefer the sessions wiki for fully-indexed sessions.** If a sessions wiki exists (built by the `build-sessions-wiki` skill; `default_wiki_dir` in `~/.claude/sessions-wiki/config.json`, or a user-supplied path), `scan` uses it automatically: sessions whose wiki page fingerprint (machine + source mtime + size across main and subagent files) still matches the live files are costed from the page's pricing-free token metrics — priced at THIS skill's current rates — instead of re-parsing the JSONL, and their records carry `"source": "wiki"` plus a `wiki_page` path. For qualitative reading, prefer the wiki page (Summary / Outcome / verbatim Prompts) over `extract` for those sessions; go to `extract` only for `"source": "jsonl"` sessions or when a page lacks request-level detail a finding needs. Pass `--no-wiki` to force source-only parsing. Dollar figures never come from the wiki — only token counts do.
...
<script_index>
All analyses run through `scripts/parse_sessions.py` (Python 3, stdlib only — invoke with plain `python`, no `rtk` prefix since RTK has no filter for this script's output shape). Parsing/dedup/token-math/fingerprint/gap-detection are **shared** with the build-sessions-wiki skill via `session_core.py`, which `parse_sessions.py` imports from `../../build-sessions-wiki/scripts/` (that skill must sit beside this one); `parse_sessions.py` adds only pricing and the scan/report layer on top. The JSONL and wiki cost paths both go through one `record_from_core_metrics` / `price_token_buckets`, so they cannot disagree. Tests: `python -m unittest discover -s skills/session-retro/tests -p "test_*.py"`.

- `python scripts/parse_sessions.py scan [--claude-dir DIR] [--project SUBSTRING ...] [--since YYYY-MM-DD] [--out DIR] [--top N] [--sonnet5-intro] [--pricing-file JSON] [--wiki-dir DIR | --no-wiki]` — walks `~/.claude/projects/**/*.jsonl`, aggregates every session, and writes `metrics.json` + `summary.md` to `~/.claude/session-retro/runs/<YYYYMMDD-HHMMSS>/`. Run this first in every workflow. When a sessions wiki is available (explicit `--wiki-dir`, or `default_wiki_dir` from `~/.claude/sessions-wiki/config.json`), fully-indexed sessions are costed from wiki token metrics at current rates instead of re-parsing their JSONL; `metrics.json` reports the split as `sessions_from_wiki` / `sessions_from_jsonl` and each session record carries `source: wiki|jsonl`.
